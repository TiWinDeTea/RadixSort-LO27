...
void PrintList(BaseNIntegerList l)
{
	...
	unsigned short i, j = 0;	/* j : counter for the number of elements currently on the screen */
	...
	--term_height;
	cursor_shift = 1 + (b+1) * max_val_size / b + (max_val_size%b != 0);
	...
	while (elem != NULL && user_input != 'q')
	{
		...
		if (j < term_height)
		{
			...
			for (i = tmp; i-- ;)	/* We are in Little Endian */
			{
...
unsigned char Menu(const char* choices, unsigned char nb_choices, char* text_color, char* bg_color, unsigned char choice)
{
	...
	do
	{
		...
		if ((key_pressed == IO_SPECIAL_CHAR && InstantGetChar() == '[') || key_pressed == '[') /* pressing an arrow inputs both SPECIAL, [, and a letter ; pressing pge up/down inputs [, a number, and ~ */
		{
			/* deselecting text */
			key_pressed = InstantGetChar();
			SetCursorPos(x_text[choice], y_text[choice]);
